	...
	for (int i = 0; i < count; ++i) {
		...
		if (lookupType(tokens[i], &type, name)) {
			...
			if (type == PARAM_TYPE_INT) {
			  ...
			  Spectrum spec((float*)data);
			  ps.AddSpectrum(name,&spec , nItems / COLOR_SAMPLES);
