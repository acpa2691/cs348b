...
// NewKdTreeAccel Declarations
struct KdAccelNode;
class  NewKdTreeAccel : public Aggregate {
public:
	// NewKdTreeAccel Public Methods
	NewKdTreeAccel(const vector<Reference<Primitive> > &p,
		int icost, int scost,
...
// NewKdTreeAccel Method Definitions
NewKdTreeAccel::
    NewKdTreeAccel(const vector<Reference<Primitive> > &p,
		int icost, int tcost,
		float ebonus, int maxp, int maxDepth)
	...
	// Initialize mailboxes for _NewKdTreeAccel_
	curMailboxId = 0;
...
void NewKdTreeAccel::buildTree(int nodeNum,
...
bool NewKdTreeAccel::Intersect(const Ray &ray,
...
bool NewKdTreeAccel::IntersectP(const Ray &ray) const {
	// Compute initial parametric range of ray inside kd-tree extent
	float tmin, tmax;
	if (!bounds.IntersectP(ray, &tmin, &tmax))
		return false;
...
extern "C" DLLEXPORT Primitive *CreateAccelerator(const vector<Reference<Primitive> > &prims,
	...
	float emptyBonus = ps.FindOneFloat("emptybonus", 0.85f);
	int maxPrims = ps.FindOneInt("maxprims", 1);
	int maxDepth = ps.FindOneInt("maxdepth", -1);
	
	ProgressReporter progress(1, "Building new KDTree");
	
	NewKdTreeAccel * accel = new NewKdTreeAccel(prims, isectCost, travCost,
