...
public:
	...
	Spectrum f(const Vector &woW, const Vector &wiW,
		BxDFType flags = BSDF_ALL) const;
	Spectrum * f_ptr(const Vector &woW, const Vector &wiW,
		BxDFType flags = BSDF_ALL) const;
...
public:
	...
	virtual Spectrum * f_ptr(const Vector &wo,
...
public:
	...
	virtual void Sample_derived(const Vector &wi, Vector *sampled_f, float u1, float u2, float *pdf) const;
	virtual void Sample_uniform(const Vector &wi, Vector *sampled_f, float u1, float u2, float *pdf) const;
	virtual void Sample_cosine(const Vector &wi, Vector *sampled_f, float u1, float u2, float *pdf) const;
	
	virtual float Pdf(const Vector &wi, const Vector &wo) const;
	virtual float Pdf_derived(const Vector &wi, const Vector &wo) const;
	virtual float Pdf_uniform(const Vector &wi, const Vector &wo) const;
	virtual float Pdf_cosine(const Vector &wi, const Vector &wo) const;
private:
	float exponent;
};
class COREDLL SineDistribution : public MicrofacetDistribution {
public:
	SineDistribution(float e) { if (e > 1000.f || isnan(e)) e = 1000.f; exponent = e; }
	// SineDistribution Public Methods
	float D(const Vector &wh) const {
		float costhetah = fabsf(CosTheta(wh));
		float sinthetah = sqrtf(max(0.f, 1-costhetah*costhetah));
		float value = (exponent+2) * INV_TWOPI * powf(sinthetah, exponent);
		//printf("Sine cos value: %f sin value: %f final value: %f\n", costhetah, sinthetah, value);
		return value;
	}
	virtual void Sample_f(const Vector &wi, Vector *sampled_f, float u1, float u2, float *pdf) const;
	virtual void Sample_derived(const Vector &wi, Vector *sampled_f, float u1, float u2, float *pdf) const;
...
class COREDLL Velvety : public MicrofacetDistribution {
public:
	Velvety(float e) { if (e > 1000.f || isnan(e)) e = 1000.f; exponent = e; }
	// SineDistribution Public Methods
	float D(const Vector &wh) const {
		float costhetah = fabsf(CosTheta(wh));
		float sinthetah = fabsf(SinTheta(wh));
		float sigma = 0.25f; //used as default
		float integralValue = 0.849235; //calculated with mathematica for sigma = 0.25
		float normalizer = 1.f/(2.f*M_PI*integralValue);
		float value = normalizer*(1.f+4.f*exp(-powf(costhetah/sinthetah,2.f)/sigma));
		//printf("%f\n", value);
		return value;
	}
	virtual void Sample_f(const Vector &wi, Vector *sampled_f, float u1, float u2, float *pdf) const;
