...
float Blinn::Pdf_uniform(const Vector &wo, const Vector &wi) const {
	return INV_TWOPI;
}

float Blinn::Pdf_cosine(const Vector &wo, const Vector &wi) const {
	return SameHemisphere(wo, wi) ? fabsf(wi.z) * INV_PI : 0.f;
}

void Blinn::Sample_derived(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	// Compute sampled half-angle vector $\wh$ for Blinn distribution
	float costheta = powf(u1, 1.f / (exponent+2));
...
void Blinn::Sample_uniform(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	...
	*wi = -wo + 2.f * costheta * H;
	// Compute PDF for \wi from Blinn distribution
	float blinn_pdf = Blinn::Pdf_uniform(wo, *wi);
	*pdf = blinn_pdf;
}

void Blinn::Sample_cosine(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	
	Vector H = CosineSampleHemisphere(u1, u2);
	...
	*wi = -wo + 2.f * costheta * H;
	// Compute PDF for \wi from Blinn distribution
	float blinn_pdf = Blinn::Pdf_cosine(wo, *wi);
	*pdf = blinn_pdf;
}



void SineDistribution::Sample_f(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	SineDistribution::Sample_cosine(wo, wi, u1, u2, pdf);
}

float SineDistribution::Pdf(const Vector &wo, const Vector &wi) const {
	return SineDistribution::Pdf_cosine(wo, wi);
}

float SineDistribution::Pdf_derived(const Vector &wo, const Vector &wi) const {
	...
	// Compute PDF for \wi from Blinn distribution
	float blinn_pdf = ((exponent + 2.f) *
	                   powf(sintheta, exponent)) /
		(2.f * M_PI * 4.f * Dot(wo, H));
	return blinn_pdf;
}

float SineDistribution::Pdf_uniform(const Vector &wo, const Vector &wi) const {
	return INV_TWOPI;
}

float SineDistribution::Pdf_cosine(const Vector &wo, const Vector &wi) const {
	return SameHemisphere(wo, wi) ? fabsf(wi.z) * INV_PI : 0.f;
}

void SineDistribution::Sample_derived(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	// Compute sampled half-angle vector $\wh$ for Blinn distribution
	float sintheta = powf(u1, 1.f / (exponent+2));
...
void SineDistribution::Sample_uniform(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	...
	*wi = -wo + 2.f * costheta * H;
	// Compute PDF for \wi from Blinn distribution
	float blinn_pdf = SineDistribution::Pdf_uniform(wo, *wi);
	*pdf = blinn_pdf;
}

void SineDistribution::Sample_cosine(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	...
	*wi = -wo + 2.f * costheta * H;
	// Compute PDF for \wi from Blinn distribution
	float blinn_pdf = SineDistribution::Pdf_cosine(wo, *wi);
	*pdf = blinn_pdf;
}

void Velvety::Sample_f(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	Velvety::Sample_cosine(wo, wi, u1, u2, pdf);
}

float Velvety::Pdf(const Vector &wo, const Vector &wi) const {
	return Velvety::Pdf_cosine(wo, wi);
}

void Velvety::Sample_uniform(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	...
	*wi = -wo + 2.f * costheta * H;
	// Compute PDF for \wi from Blinn distribution
	float blinn_pdf = Velvety::Pdf_uniform(wo, *wi);
	*pdf = blinn_pdf;
}

void Velvety::Sample_cosine(const Vector &wo, Vector *wi,
		float u1, float u2, float *pdf) const {
	...
	*wi = -wo + 2.f * costheta * H;
	// Compute PDF for \wi from Blinn distribution
	float blinn_pdf = Velvety::Pdf_cosine(wo, *wi);
	*pdf = blinn_pdf;
}

float Velvety::Pdf_uniform(const Vector &wo, const Vector &wi) const {
	return INV_TWOPI;
}

float Velvety::Pdf_cosine(const Vector &wo, const Vector &wi) const {
	return SameHemisphere(wo, wi) ? fabsf(wi.z) * INV_PI : 0.f;
}
